...
# Runs as root by design. Sliver binds privileged ports (53/443), writes to
# /opt/sliver via the entrypoint shim (chown of the operator-key volume),
# and needs raw socket access for the DNS+mTLS listeners. Hardening
# happens at the docker-compose layer (read-only rootfs + tmpfs +
# cap-drop ALL + cap-add NET_BIND_SERVICE,NET_RAW only). Semgrep
# ``missing-user-entrypoint`` finding is acknowledged and dispositioned.
USER root
