...
# ── Reverse Engineering: Ghidra 12.1 + radare2 + binwalk ──
#
# Ghidra: downloaded from GitHub releases, extracted to /opt/ghidra.
# The Ghidra MCP plugin (bethington/ghidra-mcp) runs as a sidecar
# container — see the ghidra-mcp service in docker-compose.yml.
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=sandbox-apt-cache \
    --mount=type=cache,target=/var/lib/apt/lists,sharing=locked,id=sandbox-apt-lists \
    apt-get update && \
    apt-get install -y --no-install-recommends --no-install-suggests \
