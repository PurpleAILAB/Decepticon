    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                log.warning("Sandbox connection failed (attempt %d/%d), retrying in %.1fs: %s",
...
class HTTPSandbox(BaseSandbox):
    ...
    def _request(self, method: str, path: str, **kwargs) -> httpx.Response:
        ...
        resp = _retry_on_connection_error(
            lambda: getattr(self._http(), method)(path, **kwargs)
        )
        try:
            resp.raise_for_status()
        except httpx.HTTPStatusError as exc:
            raise SandboxError(
                f"Sandbox returned {exc.response.status_code}: "
