...
async def _prune_old_scratch(workspace_path: str = "/workspace") -> None:
    ...
    """
    if _sandbox_var.get() is None or workspace_path == "/workspace":
        return
...
async def _offload_large_output(
    ...
    """
    assert _sandbox_var.get() is not None
    ...
    await asyncio.to_thread(_sandbox_var.get().execute, f"mkdir -p {workspace_path}/.scratch")
    await asyncio.to_thread(_sandbox_var.get().upload_files, [(filename, output.encode("utf-8"))])
