...
def test_rescan_preserves_first_seen_mttd(tmp_path: Path) -> None:
    graph = KnowledgeGraph()
    _record_hit(graph, "sandbox.tmux.main", _kerberoast_hit(detection_ts=105.0))
    first = graph.by_kind(NodeKind.DETECTION_FIRED)[0]
    assert first.props["mttd_seconds"] == 5.0

    # Same logged event re-detected far later — timing must not inflate.
    _record_hit(graph, "sandbox.tmux.main", _kerberoast_hit(detection_ts=900.0))
    fired = graph.by_kind(NodeKind.DETECTION_FIRED)
    assert len(fired) == 1  # idempotent: same (rule, event_ts) key
