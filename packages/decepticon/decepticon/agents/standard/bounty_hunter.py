"""Bounty Hunter Agent — local vulnerability research for bug bounty programs.

Specialises in turning compiled binaries, shared libraries, container
runtimes, and drivers into exploitable findings through a systematic
fuzz → triage → exploit → report loop: harness generation, ASAN-driven
crash triage, exploitability classification, ROP gadget inventories, and
PoC development for local privilege escalation and container escape.

Tool surface (fuzzing + reversing research tools + bash):
    fuzz_generate_harness, fuzz_triage_crash, fuzz_status,
...
cleanly:
  ...
  1. **OSS default**: ``create_bounty_hunter_agent()`` — no args.
...
Middleware slots (per ``SLOTS_PER_ROLE["bounty_hunter"]``):
...
from decepticon.tools.fuzzing.tools import FUZZING_TOOLS
    ...
    *,
    ...
    Args:
        ...
        llm: bound chat model. Defaults to
            ``LLMFactory().get_model("bounty_hunter")``.
        fallback_models: passed to ``ModelFallbackMiddleware``. Defaults
            to ``LLMFactory().get_fallback_models("bounty_hunter")``.
...
if is_bundle_enabled("standard"):
    graph = create_bounty_hunter_agent()


SUBAGENT_SPEC = SubAgentSpec(
    name="bounty_hunter",
    description=(
        "Local vulnerability research specialist for bug bounty programs. "
        "Use for fuzzing campaigns (AFL++/honggfuzz/libFuzzer harness "
        "generation + ASAN triage), crash triage and exploitability "
        "classification, container escape analysis, local privilege "
        "escalation, SUID/capability binary analysis, device driver and "
        "ioctl auditing, and PoC development for confirmed memory "
        "corruption. Ideal for compiled library, container runtime, "
        "and device driver bounties."
    ),
    factory=create_bounty_hunter_agent,
